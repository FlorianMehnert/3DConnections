...
public class RuntimeComponentTracker : MonoBehaviour
{
    ...
    private void AnalyzePropertyForComponentReferences(MonoBehaviour source, PropertyInfo property)
    {
        ...
        catch (Exception e)
        {
            if (logDiscoveredReferences)
            {
                Debug.LogWarning($"Error analyzing property {property.Name} in {source.GetType().Name}: {e.Message}");
            }
        }
    }

    private void ProcessPotentialComponentReference(MonoBehaviour source, object value, Type fieldType, string fieldName)
    {
        ...
        if (typeof(Component).IsAssignableFrom(fieldType) && value is Component targetComponent)
        {
            if (trackGetComponentCalls)
            {
                ...
                if (logDiscoveredReferences)
                {
                    Debug.Log($"Found component reference: {source.name}.{fieldName} -> {targetComponent.name} ({fieldType.Name})");
                }
            }
        }
    ...
    public static void RecordComponentReference(GameObject source, Component target, string methodType, Type componentType)
    {
    ...
    private void CreateEdgeConnection(GameObject sourceNode, GameObject targetNode, Color color, string connectionType)
    {
    ...
    public void PerformDeepAnalysis()
    {
        ...
        Debug.Log($"Deep analysis complete. Discovered {discoveredReferences} component references in {(endTime - startTime):F2} seconds.");
    }
}
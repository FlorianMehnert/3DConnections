    ...
    private List<EventSubscription> AnalyzeSourceCodeForEventSubscriptions(string sourceCode, Type subscriberType)
    {
        ...
        try
        {
            ...
            foreach (var assignment in assignments)
            {
                ...
                results.AddRange(from kvp in _eventPublishers let publisherType = kvp.Key where kvp.Value.Contains(fieldName) select new EventSubscription { SubscriberType = subscriberType, EventFieldName = fieldName, PublisherType = publisherType });
            }
        }
        catch (Exception e)
        {

...
namespace _3DConnections.Runtime.Managers
{
    ...
    public class NodeProximityController : MonoBehaviour
    {
        ...
        [Tooltip("Amount of distance the mouse has to move before everything is recalculated")] 
        ...
        private void Update()
        {
            if (!enableEffect || Time.time - _lastUpdateTime < UpdateInterval)
                return;
        ...
        {
            return _cachedNodes == null || 
                   _cachedNodes.Count != nodeGraph.NodeCount ||
                   _nodePositions == null ||
                   _nodePositions.Length != nodeGraph.NodeCount;
        }
        
        private void UpdateNodeData()
        {
            _cachedNodes = new List<GameObject>(nodeGraph.AllNodes);
            int nodeCount = Mathf.Min(_cachedNodes.Count, maxNodes);
            ...
            for (int i = 0; i < nodeCount; i++)
            {
                ...
                _scaleFactors[i] = 1f; // Start with a normal scale
            }

...
namespace _3DConnections.Runtime.Exporters
{
    using System.Linq;
    ...
    public class MermaidExporter : MonoBehaviour
    {
        ...
        private static void ExportToMermaid(string path)
        {
            ...
            foreach (var edge in from connection in ScriptableObjectInventory.Instance.conSo.connections
                     where connection.startNode && connection.endNode != null
                     let startName = connection.startNode.name
                     let endName = connection.endNode.name
                     let label = string.IsNullOrEmpty(connection.connectionType)
                         ? ""
                         : $" |{connection.connectionType}|"
                     select $"{Sanitize(startName)} -->{label} {Sanitize(endName)}"
                     into edge
                     where seenConnections.Add(edge)
                     select edge)
            {
                sb.AppendLine(edge);
            }

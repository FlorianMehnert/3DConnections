...
public class ParentChildConnections : MonoBehaviour
{
    ...
    /// <summary>
    /// Prints all node connections to the console (for debugging)
    /// </summary>
    private void PrintNodeConnections()
    {
        var connections = CalculateNodeConnections();
        
        foreach (var nodeConnection in connections)
        {
            Debug.Log($"Parent Node: {nodeConnection.Key.Name}");
            Debug.Log("Connected Nodes:");
            
            foreach (Node childNode in nodeConnection.Value)
            {

...
namespace _3DConnections.Runtime.Simulations
{
    ...
    public class SpringSimulation : MonoBehaviour, ILogable
    {
        ...
        private void Update()
        {
            ...
            for (var i = 0; i < _nodes.Count; i++)
            {
                _nodes[i].transform.position =
                    new Vector3(_positions[i].x, _positions[i].y, _nodes[i].transform.position.z);
            }
        }
        ...
        {
            foreach (var col in ScriptableObjectInventory.Instance.graph.AllNodes
                         .Select(node => node.GetComponent<Collider2D>()).Where(col => col))
            {
                col.isTrigger = true;
            }
        }
